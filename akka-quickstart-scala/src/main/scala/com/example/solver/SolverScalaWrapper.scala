...
object SolverScalaWrapper {
  ...
  def calcSolutions(node: TreeNode): List[Map[Int, String]] = {
    ...
    val solutions: List[Map[Int, String]] = Solver.solve(node.getFullGraphVariables.asJava, java_mapping.asJava)
      .asScala
      .toList
      .map(internal_map => internal_map.asScala.toMap map { case (key, value) => (key.toInt, value) })
    val already_solved: Map[Int, String] = node.getBareFullGraphVariables
    solutions.map { solution =>
      already_solved ++ solution
    }
  }
}
